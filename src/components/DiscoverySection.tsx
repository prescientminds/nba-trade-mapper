...
const METRIC_DEFS: Record<string, { metricLabel: string; metricExplanation: string }> = {
  ...
  chains: {
    ...
    metricExplanation:
      `How much winning value a team generated by holding an asset, flipping it, and flipping those assets again — recursively. In multi-player trades, each asset receives fractional credit so value isn\'t double-counted. ${FORMULA_BASE}`,
  },
};
      ...
      try {
        ...
        const chainCards: TradeCard[] = chainRows
          .map((row): TradeCard | null => {
            ...
            // Skip pass-throughs: team received an asset but produced zero direct value
            if (winnerData.direct < 1 && winnerData.depth <= 1) return null;
